...
public interface OrderingRepository extends JpaRepository<Ordering, Long> {
    ...
    //    취소된 주문리스트 찾기
    List<Ordering> findByStatusByIdDesc(OrderStatus status);
...
//    조인해서 매장별 필터링 / 오래된 주문이 위로
    ...
    @Query("SELECT DISTINCT o FROM Ordering o " +
            ...
            "LEFT JOIN FETCH od.orderingDetailOptions " +
...
//    특정 시간 내에 승인된 결제와 연관된 주문들을 조회
    @Query("SELECT SUM(o.totalPrice) FROM Ordering o " +
            "WHERE o.customerTable.store.id = :storeId " +
            "AND o.paymentState = 'COMPLETED' " + // PaymentState가 COMPLETED인 것만 합산
            "AND o.createTimeAt >= :openedAt " +
            "AND o.createTimeAt <= :closedAt")
    Long sumTotalRevenue(@Param("storeId") Long storeId,
