...
import com.beyond.Pocha_On.menu.dtos.MenuCreateReqDto;
import com.beyond.Pocha_On.menu.dtos.MenuUpdateReqDto;
...
public class MenuService {
    ...
    @Autowired
    public MenuService(MenuRepository menuRepository, OwnerRepository ownerRepository, StoreRepository storeRepository, MenuCategoryRepository menuCategoryRepository, HttpServletRequest request, S3Client s3Client, RedisTemplate<String, String> redisTemplate) {
...
//            이미지를 삭제
            if (menu.getMenuImageUrl() != null && !reqDto.getMenuImage().isEmpty()) {
                String imgUrl = menu.getMenuImageUrl();
                String fileName = imgUrl.substring(imgUrl.lastIndexOf("/") + 1);
                s3Client.deleteObject(a -> a.bucket(bucket).key(fileName));
            }
