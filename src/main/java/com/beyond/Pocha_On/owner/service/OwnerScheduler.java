...
import com.beyond.Pocha_On.store.domain.StoreSettlement;
import com.beyond.Pocha_On.store.repository.StoreRepository;
import com.beyond.Pocha_On.store.repository.StoreSettlementRepository;
...
public class OwnerScheduler {
    ...
    @Autowired
    public OwnerScheduler(StoreRepository storeRepository, TaskScheduler taskScheduler, OrderingRepository orderingRepository, StoreSettlementRepository storeSettlementRepository) {
...
//        현재 정산 시점(마감시간)
        LocalDateTime currentClosedAt = LocalDateTime.now();
...
//        오늘 날짜(연,월,일) + 오픈시간(시,분)
        LocalDateTime currentOpenedAt = LocalDateTime.of(currentClosedAt.toLocalDate(), openTime);
...
//        매출 합계 조회
        Long dayTotalAmount = orderingRepository.sumTotalRevenue(storeId, currentOpenedAt, currentClosedAt);

        if (dayTotalAmount == null) dayTotalAmount = 0L;
